...
class ChangelogBuilderBase:
    ...
    def _get_changes_between_releases(self) -> list[dict[str, Any]]:
    ...
    def _get_release_date(self, release) -> str:
        """Using GitHub API gets the latest release date"""
        url = (
            f"{self.GITHUB_API_URL}/repos/"
            f"{self.action_env.repository}/releases/tags/"
            f"{release}"
        )
    ...
    def build(self) -> str:
        ...
        self.change_list = self. _get_changes_between_releases()
